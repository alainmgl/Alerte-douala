...
import { zoneName, sensorTypeLabel, alertLevelLabel } from '@/utils/labels';
  ...
  return (
    <div className={styles.row}>
      <div className={styles.rowHead}>
        ...
        <span className={styles.rowLabel}>{sensorTypeLabel(type)}</span>
  ...
  return (
    ...
    >
      <header className={styles.head}>
        ...
        >
          {offline ? t('admin.sensors.card.offline') : alertLevelLabel(level)}
        </span>
      </header>
      ...
      <div className={styles.body}>
        {TYPE_KEYS.map((typeKey) => (
          <MeasureRow
            key={typeKey}
            type={typeKey}
            value={sensor.lastReading?.[typeKey]}
            threshold={sensor.thresholds?.[typeKey]}
          />
        ))}
      </div>

      <footer className={styles.foot}>
        <span>{t('admin.sensors.card.battery')} : {sensor.lastReading?.batteryLevel ?? '—'}%</span>
      </footer>
    </article>
  );
}
