...
export default function Report() {
  ...
  function onFallbackFile(e) {
    ...
    reader.onloadend = () => {
      if (typeof reader.result === 'string') setExternalPhoto(reader.result);
    };
  ...
  function validate() {
    ...
    if (!photoDataUrl) next.photo = 'Une photo est requise.';
    setErrors(next);
  ...
  async function onSubmit(e) {
    ...
    setSubmitting(true);
    try {
      const photoDataUrl = await shrinkDataUrl(photoDataUrl, {
        maxWidth: 1280,
        quality: 0.75,
      });
